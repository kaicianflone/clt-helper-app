...
import { renderDiff, escapeMd, isVerifyOnlyChange } from "./diff";

describe("renderDiff", () => {
  it("renders changed primitives as before → after", () => {
    expect(renderDiff({ lengthMiles: 16.4 }, { lengthMiles: 19.3 }))
      .toContain("`lengthMiles`: 16.4 → 19.3");
  });
  ...
  it("identifies added object-array entries by name", () => {
    ...
    const after = { pointsOfInterest: [
      { name: "Restroom", lat: 0, lng: 0 },
      { name: "Water", lat: 1, lng: 1 },
    ]};
    expect(renderDiff(before, after)).toMatch(/pointsOfInterest.*added \[Water\]/);
  });
  it("identifies removed object-array entries", () => {
    const before = { trailheads: [
      { name: "A", lat: 0, lng: 0 }, { name: "B", lat: 1, lng: 1 },
    ]};
  ...
  it("identifies modified object-array entries", () => {
    const before = { photos: [{ url: "https://x", caption: "old", attribution: "u" }] };
    const after = { photos: [{ url: "https://x", caption: "new", attribution: "u" }] };
    expect(renderDiff(before, after)).toMatch(/photos.*modified \[https:\/\/x\]/);
  });
  it("falls back to length-only diff for arrays of primitives", () => {
    expect(renderDiff({ tags: ["a"] }, { tags: ["a","b"] })).toMatch(/added 1 entr/);
  });
});
