...
const PII_PATTERNS: { pattern: RegExp; label: string }[] = [
  ...
  { pattern: /\b(?:\+?1[\s.-]?)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}\b/, label: "phone" },
];
...
export const containsObjectionableContent = (input: {
  ...
  const strings = [input.displayName, input.note, ...collectStrings(input.patch)];
  for (const s of strings) {
    if (!s) continue;
    if (leoProfanity.check(s)) return { violation: true, reason: "profanity" };
    for (const { pattern, label } of PII_PATTERNS) {
      if (pattern.test(s)) return { violation: true, reason: `potentially identifies a person (${label})` };
    }
  }
