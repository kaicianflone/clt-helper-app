    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!c.checkRateLimit) {
        throw new TRPCError({
          ...
          message: "Rate limit service not configured.",
        });
      }
      ...
      if (!limited.ok) {
        throw new TRPCError({
          ...
          message:
            "Rate limit reached: too many submissions. Try again tomorrow.",
        });
      }

      // Content filter — use ctx override if provided (for testing), else the real module
      const contentFilter =
        c.containsObjectionableContent ?? defaultContentFilter;
      ...
      if (!c.fetchFileFromRepo) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Repo fetch service not configured.",
        });
      }
      ...
      const verifyOnly =
        exists && c.isVerifyOnlyChange?.(current, validated) === true;

      // Human-readable diff for PR body
      if (!c.renderDiff) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Diff service not configured.",
        });
      }
      ...
      if (!c.openCommunityPR) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "PR service not configured.",
        });
      }
